...
public class GuiScaleSlider extends GuiButton {
	...
	public GuiScaleSlider(int par1, int par2, int par3, double maxDistance,
	...
	private void updateDisplayString() {
		this.displayString = (this.name + ": " + ((this.sliderValue >= 1.0F)
				&& (this.infinityAllowed) ? "infinity" : Float
				.valueOf((float) Math.round(this.maxScale * this.sliderValue
						* 10.0D) / 10.0F)));
	}

	private void adjustFromScale(float scale) {
		this.sliderValue = (scale / (float) this.maxScale);
	...
	public float getScale() {
		...
		return (float) Math.round(this.maxScale * this.sliderValue * 10.0D) / 10.0F;
	}
}
 ...
 * 
 * Qualified Name: shadowjay1.forge.simplelocator.gui.GuiScaleSlider
 * 
